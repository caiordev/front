...
export default {
  ...
  setup() {
    ...
    const matricularAluno = async (idDisciplina: string) => {
      const vagasDisciplinaAtual = disciplinas.value.find(
    ...
    const setaDisciplinasUser = async () => {
      ...
      const matriculasFiltradas = result.filter(
        (matricula) => matricula.ID_USUARIO === userAluno.value.ID
      );
      ...
      const disciplinasFiltradas = disciplinas.value.filter((disciplina) =>
        matriculasFiltradas.some(
          (matricula) => matricula.ID_DISCIPLINA === disciplina.ID
        )
      );
      ...
      state.disciplinasNaoMatriculadas = disciplinas.value.filter(
        (disciplina) =>
          !disciplinasUser.value.some((d) => d.ID === disciplina.ID)
      );
    };
